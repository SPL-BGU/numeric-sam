...
from pddl_plus_parser.models import Domain, Observation, PDDLObject
...
def barman_observation(barman_domain: Domain) -> Observation:
    barman_problem = ProblemParser(problem_path=BARMAN_ERROR_IN_PRODUCTION_PROBLEM_PATH, domain=barman_domain).parse_problem()
...
def test_create_state_discrete_snapshot_creates_a_snapshot_with_negative_predicates_set_that_is_not_empty(
    ...
    observed_objects = list(elevators_observation.grounded_objects.values())
    predicates = elevators_environment_snapshot.create_discrete_state_snapshot(state=initial_state, relevant_objects=observed_objects)
    negative_predicates = [p for p in predicates if not p.is_positive]
    assert len(negative_predicates) > 0


def test_create_state_discrete_snapshot_where_objects_are_derived_from_actions_returns_correct_literals(
    elevators_environment_snapshot: EnvironmentSnapshot, elevators_observation: Observation, elevators_domain: Domain
):
    observation_component = elevators_observation.components[0]
    initial_state = observation_component.previous_state
    executed_action = observation_component.grounded_action_call
    executed_action_signature = elevators_domain.actions[executed_action.name].signature
    observed_objects = [
        PDDLObject(name=param_name, type=param_type)
        for param_name, param_type in zip(executed_action.parameters, executed_action_signature.values())
    ]
    predicates = elevators_environment_snapshot.create_discrete_state_snapshot(state=initial_state, relevant_objects=observed_objects)
    for predicate in predicates:
        assert set(predicate.grounded_objects).issubset(executed_action.parameters)
...
def test_create_state_discrete_snapshot_creates_a_snapshot_with_correct_object_types_for_objects_with_type_hierarchy_in_barman_domain(
    ...
    observed_objects = list(barman_observation.grounded_objects.values())
    predicates = barman_environment_snapshot.create_discrete_state_snapshot(state=pre_state, relevant_objects=observed_objects)
...
def test_create_snapshot_when_should_not_include_all_objects_creates_lower_number_of_predicates_than_when_with_all_objects(
    ...
    elevators_environment_snapshot.create_triplet_snapshot_for_universal_variables(
...
def test_create_triplet_snapshot_returns_the_number_of_numeric_functions_that_match_the_action_that_is_currently_being_executed(
    ...
    assert (
        len(minecraft_environment_snapshot.previous_state_functions.keys()) == 8
    )  # one with the action parameter, one with const, and 6 with no parameters at all.
